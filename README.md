...
`graph` builds the paper's tiny topologies (path-star, grid, cycle,
irregular, tree-star, complete) over dense nalgebra adjacency, and `spectral`
provides the row-normalized transition matrix D⁻¹A, the random-walk Laplacian
L = (I − D⁻¹A) + (I − D⁻¹A)ᵀ, and `Spectrum` — the eigendecomposition of −L
with descending order, a deterministic sign convention, and degenerate-group
detection. Closed-form pins (15-cycle, complete graphs) and property pins run
in-module; public-seam integration tests live in `tests/`. Decision labels
D1–D10 are recorded in
